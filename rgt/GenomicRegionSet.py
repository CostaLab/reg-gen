...
class GenomicRegionSet:
    ...
    def get_genome_data(self,path=GenomeData.CHROMOSOME_SIZES, chrom_M=False):
        """ Add genome data from database into the GenomicRegionSet. """
        chromosome_file = open(path,'r')
    ...
    def random_regions(self, organism=GenomeData.CHROMOSOME_SIZES, total_size=None, multiply_factor=0, overlap_result=False, overlap_input=False, chrom_M=False):
        ...
        chrom_map.get_genome_data(organism)
    ...
    def projection_test(self, query, path=GenomeData.CHROMOSOME_SIZES):
        """" Return the p value of binomial test. """
        chrom_map = GenomicRegionSet("Genome")
        chrom_map.get_genome_data(path=path)
