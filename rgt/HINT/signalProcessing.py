...
class GenomicSignal:
    ...
    def bias_correction(self, signal, bias_table, genome_file_name, chrName, start, end, forward_shift, reverse_shift):
        ...
        currStr = str(fastaFile.fetch(chrName, p1_wk+forward_shift, p2_wk+forward_shift)).upper()
        currRevComp = AuxiliaryFunctions.revcomp(str(fastaFile.fetch(chrName,p1_wk+reverse_shift, p2_wk+reverse_shift)).upper())
