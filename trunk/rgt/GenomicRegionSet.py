...
class GenomicRegionSet:
    ...
    def random_regions(self, organism, total_size=None, multiply_factor=0, overlap_result=False, overlap_input=False, chrom_M=False):
        ...
        if type(total_size) == int:
            ...
            result_size = int(multiply_factor * input_num)
        ...
        if overlap_input == False:
            ...
            if overlap_result == False:
                ...
                for length in result_list:
                    cont_loop = True
                    while cont_loop:
                        sample_chrom = random.choice(chrom_list)
                        sample = random.choice([s for s in result_map.sequences if s.chrom == sample_chrom])
                        random_region = GenomicRegion(chrom=sample.chrom,
                                                      initial=sample.initial,
                                                      final=sample.final)
                        try:
                            random_posi = random.randint(random_region.initial,random_region.final - length)
                            if random_region.initial <= random_posi <= random_region.final - length:
                                cont_loop = False
                        except:
                            pass
                    random_region.initial = random_posi
                    random_region.final = random_posi + length
                    z.add(GenomicRegion(chrom=random_region.chrom,
                                        initial=random_region.initial,
                                        final=random_region.final))
                    x = GenomicRegionSet("temp")
                    x.add(random_region)
                    subtracted = result_map.subtract(x)
                    result_map = subtracted
